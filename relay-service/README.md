...
```bash
cd relay-service
/mnt/vol-eltaah12/finaagent/.venv/bin/python -m pytest -v          # 32 项（含真实 opencode 集成），约 3-5 分钟
...
- 地址：`https://<服务器>:8788/`（内网 `https://10.189.51.32:8788/`），口令登录
  （`RELAY_DASHBOARD_PASSWORD`，现值在 relay.env）
