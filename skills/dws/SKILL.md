...
`dws` 装在 `/usr/bin/dws`，登录身份以本机 `dws auth status` 为准（机队各机为京博控股集团大脑账号：.23 现为 **KG经营助理**/907021519，即控股经营助理，通讯录昵称"白露"；.29 现为 **新能源经营利润运营大脑**/98427540）。命令形如 `dws <服务> <子命令> [flags]`，`+` 开头的是封装好的快捷命令（自动解析姓名→userId），不带 `+` 的是原子命令。
