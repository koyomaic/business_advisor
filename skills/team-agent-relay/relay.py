...
用法:
  python3 relay.py config [set TOKEN]
...
配置: 环境变量 TEAM_AGENT_SERVER/TEAM_AGENT_TOKEN 优先，
其次 ~/.team-agent/config（SERVER= / TOKEN= 两行）。
...
def load_cfg() -> dict:
    ...
    if os.path.isfile(CFG_PATH):
        ...
        for i, line in enumerate(lines):
            ...
            if k == "SERVER":
                if v == OLD_DEFAULT_SERVER:
                    v = DEFAULT_SERVER
                    lines[i] = f"SERVER={v}\n"
                    changed = True
                if not cfg["server"]:
                    cfg["server"] = v
            elif k == "TOKEN" and not cfg["token"]:
                cfg["token"] = v
        if changed:
            try:
...
def cmd_config(args, cfg):
    if args.set_token:
        server = args.server or cfg["server"] or DEFAULT_SERVER
        os.makedirs(os.path.dirname(CFG_PATH), exist_ok=True)
        with open(CFG_PATH, "w", encoding="utf-8") as f:
            f.write(f"SERVER={server}\nTOKEN={args.set_token}\n")
        os.chmod(CFG_PATH, 0o600)
        out({"saved": True, "server": server, "path": CFG_PATH})
        return
    out({
...
def main() -> None:
    ...
    p = sub.add_parser("run", help="提交并等待终态（一步到位，最常用）")
    p.add_argument("description")
    add_task_args(p, waitable=True)
    p.set_defaults(fn=cmd_run)

    p = sub.add_parser("submit", help="仅提交，返回 task_id")
    p.add_argument("description")
    add_task_args(p)
    ...
    p = sub.add_parser("stream", help="SSE 实时流，逐行 JSON 到终态")
    p.add_argument("id", type=int)
